...
| Check_Out_Time__c | DateTime | Horário da saída, nunca anterior à entrada. |
| Status__c | Picklist restrita | Expected · CheckedIn · CheckedOut · NoShow · Cancelled |
| Visit_Purpose__c | Picklist restrita | Meeting · Interview · Delivery · Maintenance · Other; obrigatório. |
| Badge_Number__c | Text(10) | Crachá; opcional. |
| Company__c | Text(100) | Empresa; opcional. |
| Guest_Email__c | Email | E-mail; opcional. |
| Visit_Duration_Minutes__c | Formula: Number(1 decimal) | Minutos entre entrada e saída; em branco antes da saída. |

## 1. Abrir o Vibes e confirmar a org

**Preparar · Planejar**

Antes de gerar arquivos, confirme o projeto aberto, as ferramentas disponíveis e a org que receberá as alterações.

```text
Vamos construir um sistema de gestão de visitantes neste projeto Salesforce.
...
**Se travar:** Se a org não aparecer, confira a autorização e selecione-a novamente no projeto. Para MCP desconectado, use o diagnóstico do pre-work. Não continue com uma org escolhida por suposição.

## 2. Criar a base do aplicativo

**Construir · Executar**

Crie o contêiner de navegação. A aba de visitantes será acrescentada depois que o objeto existir.

```text
Crie ou atualize o Lightning App "Guest Management App", API Guest_Management_App, para uma recepção acompanhar visitantes.
...
**Se travar:** Se uma aba ainda inexistente bloquear o deploy, remova essa referência e tente novamente. Se o app não aparecer, confira visibilidade no perfil/permission set e atualize a sessão.

## 3. Definir o contrato de dados

**Construir · Planejar → executar**

Use o mesmo vocabulário em dados, automação, interface e agente. Os dez campos de negócio estão detalhados abaixo.

```text
Planeje o objeto Guest_Check_In__c (rótulo Guest Check-In), com Name do tipo Text para o nome do visitante, e estes 9 campos personalizados:
...
**Se travar:** Confira valores de API das picklists, não apenas seus rótulos. Um lookup para Contact não substitui Host__c → User. Não recrie o objeto para corrigir um campo isolado.

## 4. Criar e atribuir o permission set

**Construir · Executar**

Garanta que o participante possa usar o app e seus dados. O acesso ampliado fica restrito ao objeto do laboratório.

```text
Crie ou atualize o permission set "Guest Management App Access", API Guest_Management_App_Access.
...
**Se travar:** A criação do permission set não equivale à atribuição. Confira Permission Set Assignments. Se o XML falhar, valide os nomes de API e mantenha a descrição em até 255 caracteres.

## 5. Criar dados que ajudem a testar

**Construir · Planejar → executar**

Prepare uma amostra pequena e reconhecível. Ela será a referência para conferir a UI e as respostas do agente.

```text
Planeje 8 visitas fictícias em Guest_Check_In__c, identificadas pelos crachás WKS-001 até WKS-008. Use nomes fictícios e, se incluir e-mail, o domínio example.invalid. Use meu usuário ativo como anfitrião, obtendo seu Id por consulta.
...
**Se travar:** Deploy não cria dados por si só. Peça uma consulta real ao objeto. Se você já usou a amostra nos testes, os status podem ter mudado: não repita a carga para “corrigir” a contagem.

## 6. Implementar regras e testes Apex

**Construir · Planejar → executar**

A regra de negócio precisa valer para a tela, a API e o agente. Centralize-a em Apex e cubra os casos de falha.

```text
Planeje um trigger e handler Apex para Guest_Check_In__c, com processamento em lote:
...
**Se travar:** Leia o primeiro erro de compilação antes de regenerar tudo. Em Apex, literais de texto usam aspas simples. User.Name é somente leitura: testes devem preencher FirstName e LastName quando criarem usuários.

## 7. Construir uma interface utilizável

**Construir · Planejar → executar**

Dê à recepção uma visão clara de quem está no prédio, com ações que possam ser conferidas imediatamente.

```text
Planeje o LWC guestReception e seu controller Apex para o Guest_Management_App.
...
**Se travar:** LWC implantado ainda precisa estar numa página e numa aba acessível. Se a FlexiPage falhar, compare o XML gerado com um exemplo válido da mesma API; não repita o mesmo deploy sem corrigir o arquivo.

## 8. Definir o comportamento do agente

**Conectar · Planejar → executar**

Defina as quatro capacidades do agente e seus limites antes de conectar operações que alteram registros.

```text
Planeje um Employee Agent "Guest Management Agent", API Guest_Management_Agent, para apoiar a recepção em português.
...
**Se travar:** Se Employee Agent ou o editor necessário não estiver disponível, pare neste checkpoint e verifique o provisionamento com o facilitador. Não troque silenciosamente por um agente de outro tipo. Erro no bundle exige comparar o descriptor com o formato suportado pela versão instalada.

## 9. Ligar o agente aos registros reais

**Conectar · Planejar → executar**

Conecte as ações e preserve o registro correto entre turnos. Esta é a etapa mais importante para evitar uma demonstração que apenas parece funcionar.

```text
Implemente e conecte as ações do Guest_Management_Agent conforme o plano aprovado:
...
**Se travar:** Se o agente gerar Id inválido, inspecione entrada, saída e vínculo da ação; mais texto no prompt pode não resolver. Para notificação, confira o tipo e o destinatário reais. No projeto DX, o arquivo do tipo fica em notificationtypes/Guest_Arrival.notiftype-meta.xml. Confira o formato na referência técnica.

## 10. Dar acesso e ativar a versão certa

**Conectar · Executar**

Publicar, ativar e permitir acesso são operações diferentes. Confira as três antes dos testes de conversa.

```text
Atualize Guest_Management_App_Access com acesso às classes Apex realmente usadas pelo LWC e pelas ações do Guest_Management_Agent. Não inclua classes inexistentes ou de teste.
...
**Se travar:** Se o agente está publicado mas não aparece, confira ativação e acesso separadamente. No permission set, não substitua agentName por um elemento inventado. No preview, identifique se está testando um rascunho ou a versão ativa.

## 11. Testar a conversa e a mudança na org

**Validar · Conversa + evidências**

Execute a matriz de testes abaixo no agente da aplicação. O Vibes ajuda a preparar os dados e verificar o resultado; uma resposta fluente não basta.

```text
Ajude-me a executar a matriz de testes deste guia no Guest_Management_Agent.
...
**Se travar:** Separe erro de entendimento, erro de permissão e erro da ação usando o trace. Compare inputs e outputs com o registro consultado. Se o envio foi simulado em teste unitário, ainda falta conferir o sino na org.

## 12. Corrigir a causa, depois repetir o teste

**Validar · Planejar → executar**

Faça uma alteração pequena por vez e repita também o que já passava. Conversas com vários turnos expõem erros que testes isolados não mostram.

```text
Revise apenas os testes que falharam na etapa 11. Para cada um, mostre a evidência e classifique a causa: instrução, metadado, permissão, resolução de registro, ação ou estado da conversa.
...
**Se travar:** Se uma correção regredir outra ação, retorne à última versão funcional registrada, preserve a evidência da falha e corrija o vínculo específico. Teste em conversa nova para não confundir estado antigo com código novo.

## 13. Escolher uma melhoria e demonstrar

**Validar · Explorar**

Conclua com uma melhoria pequena e verificável. O sistema básico já deve funcionar antes de abrir novas frentes.

```text
O fluxo principal do Guest Management App passou pela matriz de testes. Proponha uma melhoria de escopo pequeno usando os componentes que já existem: filtro por anfitrião, busca de visitantes, visualização da duração das visitas concluídas ou prevenção de crachá duplicado entre visitas presentes.
...
**Se travar:** Se a extensão ameaçar o tempo da aula, preserve o sistema principal funcional e documente a ideia para depois. MuleSoft e OmniStudio são trilhas separadas: exigem provisionamento próprio e não são pré-requisito de conclusão deste workshop.
